import streamlit as st
import requests
from PIL import Image
import os

API_URL = "http://localhost:5000"


def run():

    # If refresh requested
    if st.session_state.get("refresh"):
        st.session_state["refresh"] = False
        st.rerun()

    # PAGE TITLE
    st.markdown(
        "<h1 style='font-size:40px; font-weight:700;'>Live Lost & Found Detections</h1>",
        unsafe_allow_html=True
    )

    # Fetch data from API
    def fetch_events():
        try:
            res = requests.get(f"{API_URL}/api/detections")
            return res.json()
        except:
            return []

    # Update collected in backend
    def update_collected(id, new_value):
        try:
            requests.post(
                f"{API_URL}/api/update_collected/{id}",
                json={"collected": new_value}
            )
        except:
            pass

    events = fetch_events()

    if not events:
        st.warning("No events found or backend is not running.")
        st.button("Refresh", on_click=lambda: st.session_state.update({"refresh": True}))
        return

    for item in events:
        st.markdown("<hr>", unsafe_allow_html=True)

        header_left, header_right = st.columns([4, 1])

        with header_left:
            st.markdown(
                f"<h2 style='margin-bottom:5px; font-weight:700;'>{item['label']}</h2>",
                unsafe_allow_html=True
            )

        with header_right:
            st.markdown("<div style='height:10px;'></div>", unsafe_allow_html=True)
            st.checkbox(
                "Collected",
                value=item.get("collected", False),
                key=item["_id"],
                on_change=update_collected,
                args=(item["_id"], not item.get("collected", False))
            )

        left, right = st.columns([1.5, 1])

        with left:
            st.write(f"**Device:** {item['device_id']}")
            st.write(f"**Object ID:** {item['object_id']}")
            st.write(f"**Detected At:** {item['timestamp']}")
            st.write(f"**Saved At:** {item['saved_at']}")

        with right:
            img_path = item["image_path"]
            if os.path.exists(img_path):
                img = Image.open(img_path)
                st.image(img, width=350)
            else:
                st.error("Image not found.")

    # Manual Refresh Button
    st.button("Refresh Live Data", on_click=lambda: st.session_state.update({"refresh": True}))
